...
import { loadWorkflow, listWorkflows, clearWorkflowCache } from "../../src/core/workflow-loader.js";
...
describe("loadWorkflow", () => {
    ...
    it("throws when module has no run export", async () => {
        const cwd = makeCwd();
        await writeFile(
            join(globalWorkflowDir, "bad-module.js"),
    ...
    it("prefers local workflow over global with same name", async () => {
        const cwd = makeCwd();
        await writeFile(
            join(localWorkflowDir, "override.js"),
            "export async function run() { return 'local'; }",
        );
        await writeFile(
            join(globalWorkflowDir, "override.js"),
            "export async function run() { return 'global'; }",
        );
    ...
    it("prefers .js over .mjs over .cjs over .ts for same name", async () => {
        const cwd = makeCwd();
        await writeFile(
            join(globalWorkflowDir, "ext-pref.mjs"),
            "export async function run() { return 'mjs'; }",
        );
        await writeFile(
            join(globalWorkflowDir, "ext-pref.js"),
            "export async function run() { return 'js'; }",
        );
...
describe("clearWorkflowCache", () => {
    it("clears the internal cache map", async () => {
        const cwd = makeCwd();
        await writeFile(
            join(globalWorkflowDir, "cache-clear.js"),
