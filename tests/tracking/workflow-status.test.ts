...
function tmpDir(): string {
    return path.join(
        os.tmpdir(),
        `workflow-status-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
}
    ...
    describe("save / load round-trip", () => {
        it("restores all fields through save and load", async () => {
            ...
            tracker.taskTracker.addTask(
                { ...makeTask({ id: "task-b", dependencies: ["task-a"] }), status: undefined }
            );
        ...
        it("save creates the workDir if it doesn't exist", async () => {
            ...
            const raw = await fs.readFile(
                path.join(nestedDir, "workflow-state.json"),
                "utf-8"
            );
