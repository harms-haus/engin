...
// via `formatWorkflowSummary(projection.agents, totalDurationMs)` and append
...
//   • they are computed from the POST-evolve projection.agents (so agents /
...
// '🎉 Complete …' line (deterministic from totalDurationMs + agentCount) and
...
import type { AgentEntity, EventRecord, EventType, WorkflowProjection } from '@engin/shared/event-types';
...
describe('ClientStore.applyEvents — workflow_completed emits summary lines', () => {
  it('appends the two summary lines alongside the completion line', () => {
    const events: EventRecord[] = [
      ...
      ev('agent_spawned', { profile: 'coder' }, { agentId: 'a1', taskId: 't1', timestamp: T0 }, 2),
      ev('turn_ended', { tokens: { input: 1200, output: 300 } }, { agentId: 'a1', taskId: 't1' }, 3),
      ev('agent_completed', {}, { agentId: 'a1', taskId: 't1', timestamp: ts(30) }, 4),
      ev('workflow_completed', { totalDurationMs: 60000, agentCount: 1 }, {}, 5),
    ];
    ...
    const expectedSummary = formatWorkflowSummary(projection.agents, 60000).map((line: string) => ({
      seq: 5,
      line,
    }));
  ...
  it('contains the two summary lines plus the existing 🎉 Complete line', () => {
    const events: EventRecord[] = [
      ev('agent_spawned', { profile: 'coder' }, { agentId: 'a1', taskId: 't1', timestamp: T0 }, 1),
      ev('turn_ended', { tokens: { input: 1200, output: 300 } }, { agentId: 'a1', taskId: 't1' }, 2),
      ev('agent_completed', {}, { agentId: 'a1', taskId: 't1', timestamp: ts(30) }, 3),
      ev('workflow_completed', { totalDurationMs: 60000, agentCount: 1 }, {}, 4),
    ];
    ...
    const summary = formatWorkflowSummary(fold(events).agents, 60000);

    // The completion line (deterministic from totalDurationMs + agentCount) …
    expect(lines).toContain('🎉 Complete in 60.0s (1 agents)');
  ...
  it('keys every summary entry with the completed event seq (drains together)', () => {
    ...
    const events: EventRecord[] = [
      ev('agent_spawned', { profile: 'coder' }, { agentId: 'a1', taskId: 't1', timestamp: T0 }, 1),
      ev('agent_completed', {}, { agentId: 'a1', taskId: 't1', timestamp: ts(30) }, 2),
      ev('workflow_completed', { totalDurationMs: 60000, agentCount: 1 }, {}, completedSeq),
    ];
    ...
    expect(atCompletedSeq.map((e) => e.line)).toEqual(['🎉 Complete in 60.0s (1 agents)', summary[0], summary[1]]);
  });
  ...
  it('appends summary lines AFTER the completion line (ordering)', () => {
    const events: EventRecord[] = [
      ...
      ev('agent_spawned', { profile: 'coder' }, { agentId: 'a1', taskId: 't1', timestamp: T0 }, 2),
      ev('agent_completed', {}, { agentId: 'a1', taskId: 't1', timestamp: ts(10) }, 3),
      ev('workflow_completed', { totalDurationMs: 30000, agentCount: 1 }, {}, 4),
    ];
    ...
    const lines = store.getState().workflowEventLog.map((e) => e.line);
    const summary = formatWorkflowSummary(fold(events).agents, 30000);

    const completeIdx = lines.indexOf('🎉 Complete in 30.0s (1 agents)');
  ...
  it('reflects agents + tokens spawned in the SAME batch (summary computed post-evolve)', () => {
...
describe('ClientStore.applyEvents — summary reflects agent active time', () => {
  it('sums agent active time when startedAt is present (snapshot) and completedAt lands via event', () => {
    ...
    const snapshot: WorkflowProjection = {
      ...
      seq: 10,
      currentPhaseId: 'exec',
      agents: { 'a1::t1': seedAgent },
    };
    store.applySnapshot(snapshot, 10);

    store.applyEvents([
      ev('agent_completed', {}, { agentId: 'a1', taskId: 't1', timestamp: ts(30) }, 11),
      ev('workflow_completed', { totalDurationMs: 60000, agentCount: 1 }, {}, 12),
    ]);

    const lines = store.getState().workflowEventLog.map((e) => e.line);
    // 30s agent / 60s total → 50%.
    expect(lines).toContain('📊 Tokens: ↑1.2k in · ↓300 out');
    expect(lines).toContain('⏱ Time: 60.0s total · 30.0s agent (50%)');
  });
});
...
describe('ClientStore.applyEvents — workflow_completed summary guards', () => {
  it('does NOT emit summary lines when totalDurationMs is missing', () => {
    const events: EventRecord[] = [
      ev('agent_spawned', { profile: 'coder' }, { agentId: 'a1', taskId: 't1', timestamp: T0 }, 1),
      ev('workflow_completed', { agentCount: 1 }, {}, 2), // no totalDurationMs
    ];
    ...
    const lines = store.getState().workflowEventLog.map((e) => e.line);
    expect(lines).toEqual(['⏳ Agent a1 spawned (coder)', '🎉 Complete in 0.0s (1 agents)']);
  ...
  it('does NOT emit summary lines when totalDurationMs is zero', () => {
    const events: EventRecord[] = [
      ev('agent_spawned', { profile: 'coder' }, { agentId: 'a1', taskId: 't1', timestamp: T0 }, 1),
      ev('workflow_completed', { totalDurationMs: 0, agentCount: 1 }, {}, 2),
    ];
  ...
  it('does NOT emit summary lines when totalDurationMs is negative', () => {
    const events: EventRecord[] = [ev('workflow_completed', { totalDurationMs: -5, agentCount: 1 }, {}, 1)];
    ...
    expect(lines).toEqual(['🎉 Complete in -0.0s (1 agents)']);
    expect(lines.some((l) => l.startsWith('📊 Tokens:'))).toBe(false);
  });

  it('emits summary lines even with no agents when totalDurationMs > 0', () => {
    // The only guard is totalDurationMs > 0; an empty agent set yields 0/0.
    const events: EventRecord[] = [ev('workflow_completed', { totalDurationMs: 3456, agentCount: 0 }, {}, 1)];

    const store = new ClientStore();
    store.applyEvents(events);

    expect(store.getState().workflowEventLog.map((e) => e.line)).toEqual([
      '🎉 Complete in 3.5s (0 agents)',
      '📊 Tokens: ↑0 in · ↓0 out',
