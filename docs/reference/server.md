...
| `start_run`                 | `{ workflowName, taskPrompt, cwd, workDir?, maxConcurrent?, apiKeys? }` | Start a run. Auto-subscribes the requester. The server decides whether to use worktrees based on whether `cwd` is a git repo. |
