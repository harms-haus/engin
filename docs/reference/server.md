...
| `worktreeManager?`                  | Per-run `WorktreeManager` owning the main + per-task worktrees and the serialized merge chain. Populated asynchronously by `RunExecutor` once the main worktree exists. `undefined` while setup is in flight, and for the non-git fallback path. |
| `pendingMergeConflicts?`            | Conflicts stashed by `handleWorktreeAction('merge')` so a follow-up `'resolve'` can forward them to the resolver.                                                                                                                                |
| `worktree?`, `apiKeys?`             | Main worktree info (set alongside `worktreeManager`) and forwarded API keys.                                                                                                                                                                     |

### Methods

| Method                                            | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `startRun(msg)`                                   | Resolve `workDir` (`msg.workDir` or `getDefaultWorkDir(cwd, wfName)`); **refuse on collision** with an active `runId`; `loadWorkflow`; `EventStore.load(workDir)`; `createStoreCallbacks`; per-run `StatusBridge`; `loadEnvFiles(cwd)`; register the handle; call `onRunsChanged`; launch `workflow.run(...)` as a **fire-and-forget** async IIFE; return `{ runId, summary }` immediately (without awaiting). **Worktree setup is unconditional and asynchronous** — `RunExecutor.execute` probes `isGitRepo(cwd)`, creates the main worktree on `engin/{mainSlug}` via a `WorktreeManager`, and wires it onto the handle + `WorkflowRunOptions` (the `msg.worktree` gate is gone). Non-git cwds warn and run in-place.                      |
| `cancelRun(runId)`                                | Aborts that run's `AbortController` (cooperative). Idempotent no-op if unknown. Does **not** kill the server.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `handleWorktreeAction(runId, action)`             | Drives the two-prompt final merge UX. `action` is `'merge'` \| `'resolve'` \| `'decline'`. **merge** — squash-merge the main-wt branch into real `main` via `finalMergeToMain`; clean → `cleanup` (outcome `clean`); conflicts → outcome `conflicts` (stash conflicts, surface worktree/branch paths). **resolve** — `resolveFinalMergeConflicts` with the stashed conflicts + task prompt; resolved → `cleanup` (outcome `resolved`); failed → outcome `failed` (preserve). **decline** — `abortFinalMerge` and broadcast outcome `declined` (preserve everything; never clean up). Result broadcast as a `worktree_merge_result` ServerMessage. No-ops when the runId is unknown or the handle has no `worktreeManager` (non-git fallback). |
...
| `run_started`           | `{ runId, summary }`                                            | A new run entered the registry. `summary.worktree` is absent here — the run executor populates it asynchronously (after LLM branch generation + worktree setup) and it surfaces in subsequent `runs` list broadcasts and the terminal summary. |
...
| `worktree_merge_result` | `{ runId, outcome, cleanupError?, worktreePath?, branchName? }` | Result of a `worktree_action`; `outcome` is `clean` \| `conflicts` \| `resolved` \| `failed` \| `declined`.                                                                                                                                    |
...
| `start_run`                 | `{ workflowName, taskPrompt, cwd, workDir?, apiKeys?, runTimeoutMs? }` | Start a run. Auto-subscribes the requester. The server decides whether to use worktrees based on whether `cwd` is a git repo. `runTimeoutMs?` — optional per-run wall-clock timeout in milliseconds; when positive, the run's AbortController is aborted after this duration and the run ends as `run_failed` with a 'Run timed out' message. Unset → no timeout. |
...
| `worktree_action`           | `{ runId, action }`                                                    | `merge` \| `resolve` \| `decline` — drives the final merge UX.                                                                                                                                                                                                                                                                                                    |

`isServerMessage(data)` is a type guard checking the `type` tag (the discriminant now
includes `worktree_merge_result`). The old `terminate_server` message and the old
unscoped `resync` are gone; the old terminal signals `workflow_complete` /
`workflow_failed` are now run-scoped `run_complete` / `run_failed`. The `start_run`
`worktree?` field and the `keep` / `discard` / `pr` `worktree_action`s are gone —
worktrees are now unconditional for git repos and the final merge is driven by the
`merge` / `resolve` / `decline` actions. See [Worktrees reference](worktrees.md).
...
interface RunSummary {
  ...
  // Populated once RunExecutor has created the main worktree (git-repo runs).
  // Absent for the non-git fallback path.
...
- [Worktrees reference](worktrees.md) — the per-task worktree system, `.worktreecopy`,
  and the two-prompt final merge UX.
