...
| `agent_spawned`      | Upsert by `agentKey`. New entity: construct full `AgentEntity`, **increment `agentCount`**, stamp `startedAt = metadata.timestamp` (once, at first spawn), read `contextWindow` from `data.contextWindow` (defensively coerced to a number; non-numbers ignored), link the task step if `taskId` and `stepIndex` are present. Existing entity: preserve `log`/tokens/`toolCallCount`/`startedAt`, update `profile`/`phaseId`/`stepIndex`/`sessionId`/`sessionPath`, set `active=true`, clear `completedAt`, refresh `contextWindow` from `data.contextWindow` when present (falling back to the existing value); **does not** re-increment `agentCount`. |
