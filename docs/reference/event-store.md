...
// registry  → threaded to engine primitives (SessionScheduler, PhaseRunner, WorktreeManager)
...
| `session_completed`    | Resolve the session via `resolveSession`; set `active=false`, `status='completed'`, `completedAt = metadata.timestamp`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `session_failed`       | Resolve the session via `resolveSession`; set `status='failed'`, `active=false`. (Session-level failure; does not change task status.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `task_parked`          | Task `status='parked'`. (Transient: the scheduler parks a task whose current batch cannot start due to gate-capacity saturation; already-started siblings keep running.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `task_unparked`        | Task `status='active'`. (The scheduler resumes a parked task when a gate slot frees up.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `workflow_data_set`    | **Shallow-merge** `data` into `projection.workflowData` (initialising the record on first event). Arbitrary workflow data attached via `onWorkflowData`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
