...
| [Task pool & execution](reference/task-pool.md)  | `RunnerPool`, `SessionGate`, the `runSession` session primitive, composable runners (`singleSession`, `linearRunner`, `reviewRunner`, …), `TaskTracker`, retries, and the prompt builder.                                                                                                                          |
| [Hooks](reference/hooks.md)                      | The hook system: the `WorkflowHooks` catalog, composition rules (`observe`/`pipeline`/`first-wins`/`all-run`), `HookRegistry`, `composeHooks`, default implementations, wiring, and the `fixLoop`/final-review boundary. **Note:** `beforeStepPrompt` is renamed `beforeSessionPrompt` in the session-first model. |
