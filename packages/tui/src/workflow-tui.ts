...
export class WorkflowTUI {
  ...
  /**
   * QR overlay component pre-generated by prepareQrCode(). Shown/hidden on
   * demand via Ctrl+Q (toggleQrCode()). Null when no QR was prepared. The QR
   * is NOT rendered by default — it must be toggled on explicitly.
   */
  private qrComponent: Component | null = null;
  ...
  start(): void {
    ...
    this.inputUnsubscribe = tui.addInputListener((data: string) => {
      ...
      // Ctrl+Q: toggle the QR code overlay (show ↔ hide). Requires a
      // component prepared via prepareQrCode(); otherwise this is a no-op.
    ...
    // The QR overlay is NOT auto-shown on start — it renders only after the
    // user presses Ctrl+Q (toggleQrCode()). prepareQrCode() pre-generates the
    // component so the toggle is synchronous and free of async latency.
  }
  ...
  /**
   * Pre-generate the QR overlay component so it can be toggled on instantly
   * via Ctrl+Q (toggleQrCode()) without an async round-trip on each keypress.
   * The QR is NOT rendered by default; call this before start() and the user
   * reveals it on demand with Ctrl+Q.
   */
  async prepareQrCode(url: string): Promise<void> {
    try {
      this.qrComponent = (await createQrOverlayComponent(url)).component;
    } catch (err) {
  ...
  /**
   * Toggle the QR code overlay on (show) ↔ off (hide). Requires a component to
   * have been prepared via {@link prepareQrCode}; if none was prepared this is
   * a no-op. Bound to Ctrl+Q in the main input listener.
   */
  private toggleQrCode(): void {
    ...
    // Hidden → show (only if a component was prepared and the TUI is up).
    if (!this.qrComponent || !this.tui) return;
    this.qrHandle = this.tui.showOverlay(this.qrComponent, QR_OVERLAY_OPTIONS);
  }
