...
import type { Task, TaskStatus, WorkflowPhase, WorkflowState } from "../core/types.js";
...
export class WorkflowStatusTracker {
    ...
    advancePhase(): void {
        ...
        if (idx < 0 || idx >= PHASE_ORDER.length - 1) {
            throw new Error(
                `Cannot advance from phase "${this._currentPhase}": already at the final phase`
            );
        }
    ...
    static async load(workDir: string): Promise<WorkflowStatusTracker> {
        ...
        try {
            ...
            if (
                ...
                throw new Error(`Workflow state file not found at "${filePath}"`);
            }
            throw err;
        }
